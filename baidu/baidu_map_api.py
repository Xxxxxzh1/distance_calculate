...
if __name__ == "__main__":
    import time
    map = BaiduMap()
    map.login('please enter your ak')
    map.read_lng_lat('cities_lng_lat_baidu.csv')
    
    result_df = read_df('distance_baidu.csv')
    # 更新全量数据需要先将数据置为0
    # result_df['baidu_map_distance'] = 0 
    print("=======\n", result_df)
    ...
    for index in range(len(result_df)):
        if result_df.at[index, 'baidu_map_distance'] == 0.0:
            ...
            try:
                ...
                print('运行出错了，速速存盘保平安！')
                result_df.to_csv(os.path.join(os.path.dirname(__file__), 'distance_baidu.csv'), index=False, encoding="UTF-8-sig")
                sys.exit()
